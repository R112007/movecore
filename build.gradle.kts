...
project(":"){
    ...
    // 预先解析 runtime classpath，避免在 from(...) 的 Provider/Callable lambda 中捕获 Project，
    // 从而解决 Configuration Cache 无法序列化 Jar task 输入的问题。
    val runtimeCpFiles = configurations.runtimeClasspath.get().map { file ->
    ...
    val jar = tasks.named<Jar>("jar"){
        ...
        val checkModName = modName
        doFirst{
            if(usedMeta.asFile.reader(Charsets.UTF_8).use{Jval.read(it)}.getString("name") != checkModName) {
                throw GradleException("Mod name mismatch in `${usedMeta.asFile.name}`; please synchronize with `gradle.properties`")
            }
        }

        // 使用外部 zip 命令追加 assets，避免 Gradle zipTree 对中文文件名处理异常
        doLast {
            val jarFile = archiveFile.get().asFile

            if (!assetsDir.exists()) {
                logger.lifecycle("Assets directory not found, skipping zip append.")
                return@doLast
            }
            ...
            if (process.exitValue() != 0) {
                throw GradleException("zip command failed with exit code ${process.exitValue()}")
            }
            logger.lifecycle("Assets appended to ${jarFile.name}")
        }
    }
